...
def image_deal(image_list, method=0, bin_method=0, bin_region=0, bin_counts=0, hough_line_threshold=0, scale=5 / 137):
    """
    ...
    for x in range(items_x):
        for y in range(items_y):
            ...
            sliced_image.detect_houghlines_vertical(hough_line_threshold)
            sliced_image.parse_houghlines(scale)
...
if __name__ == '__main__':
    ...
    #
    # new_image = image[0:3333, 0:5017][:186, :186]
    #
    ...
    # test_image.detect_houghlines_vertical(100)
    ...
    #                                 bin_counts=-15, hough_threshold=1900)
    ...
    for x in range(items_x):
        for y in range(items_y):
            ...
            while True:
                flag = 0
                try:
                    size_control(sliced_image, int(first), int(second))
                except:
